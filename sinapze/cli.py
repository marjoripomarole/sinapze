...
from sinapze.config import get_settings
from sinapze.export import export_anki_deck, export_markdown_guide
from sinapze.generate import (
    filter_chunks_for_scope,
    generate_cards,
    generate_study_guide,
)
from sinapze.ingest import (
    _persist,
...
from sinapze.scope import explain_ranking, rank_sources, scope_from_frontmatter
...
def guide(exam_name: str, output_path: Path, subject: str, course_description: str) -> None:
    ...
    if not chunks:
        console.print("[red]No chunks for that scope. Run `sinapze scope` first.[/]")
        raise click.Abort()
